...
class EscPosEncoder {
  ...
  constructor() {
    ...
    _defineProperty(this, "_codepage", void 0);
   ...
   */
  _encode(value) {
    return _iconvLite.default.encode(value, this._codepage);
  }
  ...
  codepage(value) {
    ...
    let codepage;
    if (!_iconvLite.default.encodingExists(value)) {
      throw new Error('Unknown codepage');
    }
    if (value in _iconvLite.default.encodings) {
      if (typeof _iconvLite.default.encodings[value] === 'string') {
        codepage = _iconvLite.default.encodings[value];
      } else {
        codepage = value;
      }
    } else {
  ...
  barcode(value, symbology, height) {
    ...
    if (symbology in symbologies) {
      const bytes = _iconvLite.default.encode(value, 'ascii');
  ...
  qrcode(value, model, size, errorlevel) {
    ...
    const bytes = _iconvLite.default.encode(value, 'iso88591');
  ...
  image(element, width, height, algorithm, threshold) {
    ...
    const getPixel = (x, y) => {
      var _image$data;
      return (((_image$data = image.data) === null || _image$data === void 0 ? void 0 : _image$data[(width * y + x) * 4]) ?? 0) > 0 ? 0 : 1;
    };
